...
template <typename T> T readLE(const std::byte *data) {
	T value;
	std::memcpy(&value, data, sizeof(T));
	return value;
}

template <typename T> void writeLE(std::byte *data, T value) {
...
NormalizedSave normalizeForParsing(std::span<const std::byte> data) {
	...
	if (first == static_cast<uint32_t>(data.size()) &&
	    second == kSaveMagic) {
	...
	if (pos + 4 <= data.size()) {
		...
		if (candidate >= 0 && candidate <= 3) {
			...
			int32_t ctxCampaign = readLE<int32_t>(
			    data.data() + pos + kSaveContextSize);
	...
	// Build canonical buffer: size_prefix(4) + magic(4) + context(0x438) +
	// rest.
...
bool SaveFormat::load(std::span<const std::byte> data) {
	...
	try {
		...
		// Context.
		if (hasContext_) {
			parseContext(reinterpret_cast<const std::byte *>(
			    parsed.context_data));
		}
		...
		// Main block.
		parseMainBlock(reinterpret_cast<const std::byte *>(
		    parsed.main_save_block));
		...
		for (size_t i = 0; i < parsed.units.size(); ++i) {
			...
			unit.troopInfoIndex =
			    static_cast<int32_t>(ku.troop_info_index);
			...
			std::memcpy(unit.equipmentRaw.data(), ku.skill_data,
				    24);
			...
			unit.secondaryAccessory =
			    wireToSlot(ku.troop_accessory);
		...
		// Mission completion.
		for (size_t i = 0; i < kSaveMissionSlots &&
				   i < parsed.mission_completion.size();
		     ++i) {
			missionCompletion_[i] =
			    static_cast<int32_t>(parsed.mission_completion[i]);
		}

		// Current mission index.
		currentMissionIndex_ =
		    static_cast<int32_t>(parsed.current_mission_slot);

		// Remaining bytes for round-trip preservation.
		rawTail_.clear();
		if (!parsed.tail_data.empty()) {
			rawTail_.resize(parsed.tail_data.size());
			std::memcpy(rawTail_.data(), parsed.tail_data.data(),
				    parsed.tail_data.size());
		}
...
std::vector<ValidationIssue> SaveFormat::validate() const {
	...
	if (campaignIndex_ < 0 || campaignIndex_ > 3) {
		issues.push_back({Severity::Warning, "campaignIndex",
	...
	for (size_t i = 0; i < units_.size(); ++i) {
		...
		if (unit.ucd > 3) {
			issues.push_back(
			    {Severity::Warning, "ucd",
			     "UCD value outside expected range (0-3)", i});
		}

		if (unit.jobType > 42 && unit.jobType < 32) {
			// This check is intentionally loose — charinfo job
			// types go higher.
		}
	}

	if (currentMissionIndex_ < 0) {
		issues.push_back({Severity::Warning, "currentMissionIndex",
				  "Negative mission index", 0});
	}
