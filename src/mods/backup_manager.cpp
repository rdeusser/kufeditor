...
bool BackupManager::createBackup(const std::string &gameDir, AsyncTask &task) {
	...
	for (size_t i = 0; i < files.size(); ++i) {
		...
		try {
			...
			task.setError(std::string("Failed to copy ") +
				      files[i].string() + ": " + e.what());
			return false;
		}
	}
...
bool BackupManager::restoreBackup(const BackupInfo &backup,
				  const std::string &gameDir, AsyncTask &task) {
	...
	for (size_t i = 0; i < gameFiles.size(); ++i) {
		...
		task.setProgress(static_cast<float>(i) /
				     static_cast<float>(gameFiles.size()),
				 relativePath.string());

		try {
			copyFileWithDirs(gameFiles[i], destPath);
		} catch (const fs::filesystem_error &e) {
			task.setError(std::string("Failed to restore ") +
...
std::vector<BackupInfo> BackupManager::listBackups() {
	...
	for (const auto &entry : fs::directory_iterator(dir)) {
		...
		info.timestamp =
		    formatTimestamp(entry.path().filename().string());
		...
		if (fs::exists(metaPath)) {
			...
			if (gdPos != std::string::npos) {
				auto valStart = content.find(
				    '"', content.find(':', gdPos) + 1);
				if (valStart != std::string::npos) {
					auto valEnd =
					    content.find('"', valStart + 1);
