...
std::string ModManager::modsDirectory() { return getConfigDir() + "/mods"; }
...
bool ModManager::createMod(const ModMetadata &meta, const std::string &gameDir,
	...
	for (size_t i = 0; i < relativePaths.size(); ++i) {
		...
		task.setProgress(static_cast<float>(i) /
				     static_cast<float>(relativePaths.size()),
				 relativePaths[i]);

		if (!writer.addFile(diskPath, relativePaths[i])) {
			task.setError("Failed to add file: " +
				      relativePaths[i]);
			return false;
		}
	}
...
std::variant<ModInfo, std::string>
ModManager::importMod(const std::string &zipPath) {
	ZipReader reader;
	...
	std::string jsonStr(reinterpret_cast<const char *>(jsonData->data()),
			    jsonData->size());
	auto meta = parseModJson(jsonStr);
	if (!meta) {
		return std::string("Invalid or incomplete mod.json (requires "
				   "name, version, game, files)");
	}

	// Copy to mods directory.
	fs::create_directories(modsDirectory());
	std::string destPath =
	    modsDirectory() + "/" + fs::path(zipPath).filename().string();

	// Avoid overwriting: append number if needed.
	if (fs::exists(destPath) &&
	    fs::canonical(zipPath) != fs::canonical(destPath)) {
		std::string stem = fs::path(zipPath).stem().string();
		std::string ext = fs::path(zipPath).extension().string();
		int n = 1;
		while (fs::exists(destPath)) {
			destPath = modsDirectory() + "/" + stem + "_" +
				   std::to_string(n) + ext;
			++n;
		}
	}
	...
	if (!fs::exists(destPath) ||
	    fs::canonical(zipPath) != fs::canonical(destPath)) {
		std::error_code ec;
		fs::copy_file(zipPath, destPath,
			      fs::copy_options::overwrite_existing, ec);
		if (ec) {
			return std::string("Failed to copy mod to library: " +
					   ec.message());
		}
	}
...
bool ModManager::applyMod(const ModInfo &mod, const std::string &gameDir,
			  AsyncTask &task) {
	ZipReader reader;
	...
	for (size_t i = 0; i < gameEntries.size(); ++i) {
		task.setProgress(static_cast<float>(i) /
				     static_cast<float>(gameEntries.size()),
				 gameEntries[i]);
...
std::vector<ModInfo> ModManager::listMods() {
	...
	for (const auto &entry : fs::directory_iterator(dir)) {
		...
		std::string jsonStr(
		    reinterpret_cast<const char *>(jsonData->data()),
		    jsonData->size());
		auto meta = parseModJson(jsonStr);
		if (!meta) continue;
