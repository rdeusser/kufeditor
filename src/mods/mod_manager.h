...
class ModManager {
	...
	static bool createMod(const ModMetadata &meta,
			      const std::string &gameDir,
			      const std::vector<std::string> &relativePaths,
			      const std::string &outputZipPath,
			      AsyncTask &task);
	static std::variant<ModInfo, std::string>
	importMod(const std::string &zipPath);
	static bool applyMod(const ModInfo &mod, const std::string &gameDir,
			     AsyncTask &task);
