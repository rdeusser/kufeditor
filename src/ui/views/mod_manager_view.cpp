...
void ModManagerView::drawContent() {
	...
	ImGui::BeginChild("InstalledModsSidebar", ImVec2(220, height),
	...
	ImGui::BeginChild("ModManagerContent", ImVec2(0, height),
			  ImGuiChildFlags_Borders);
	drawMainContent();
	ImGui::EndChild();
}
...
void ModManagerView::drawInstalledSidebar() {
	...
	if (installedMods_.empty()) {
		...
		for (int i = 0; i < static_cast<int>(installedMods_.size());
		     ++i) {
			bool isSelected = (selectedInstalledMod_ == i);
			if (ImGui::Selectable(installedMods_[i].name.c_str(),
					      isSelected)) {
	...
	if (ImGui::BeginPopupModal("Confirm Uninstall", nullptr,
		...
		ImGui::TextDisabled(
		    "(Files in the game directory are not reverted.)");
		ImGui::Separator();
		if (ImGui::Button("Uninstall", ImVec2(120, 0))) {
			if (selectedInstalledMod_ >= 0 &&
			    selectedInstalledMod_ <
				static_cast<int>(installedMods_.size())) {
				ModManager::markUninstalled(
				    installedMods_[selectedInstalledMod_].name);
...
void ModManagerView::drawBackupsSection() {
	...
	if (ImGui::CollapsingHeader("Backups",
		...
		if (!backups_.empty()) {
			...
			if (ImGui::BeginTable("BackupsTable", 5,
				...
				for (int i = 0;
				     i < static_cast<int>(backups_.size());
				     ++i) {
					...
					ImGui::TableNextColumn();
					ImGui::Text(
					    "%s", formatBytes(backup.totalBytes)
						      .c_str());
	...
	if (ImGui::BeginPopupModal("Confirm Restore", nullptr,
				   ImGuiWindowFlags_AlwaysAutoResize)) {
		ImGui::Text("Restore this backup? This will overwrite files in "
			    "the game "
			    "directory.");
		ImGui::Separator();
		if (ImGui::Button("Restore", ImVec2(120, 0))) {
			if (pendingBackupIndex_ >= 0 &&
			    pendingBackupIndex_ <
				static_cast<int>(backups_.size())) {
	...
	if (ImGui::BeginPopupModal("Confirm Delete Backup", nullptr,
				   ImGuiWindowFlags_AlwaysAutoResize)) {
...
void ModManagerView::drawModLibrarySection() {
	...
	if (ImGui::CollapsingHeader("Mod Library",
				    ImGuiTreeNodeFlags_DefaultOpen)) {
		...
		if (!mods_.empty()) {
			...
			if (ImGui::BeginTable("ModsTable", 6,
					      ImGuiTableFlags_Borders |
						  ImGuiTableFlags_RowBg)) {
				ImGui::TableSetupColumn(
				...
				ImGui::TableSetupColumn(
				    ...
				    100.0f);
				ImGui::TableSetupColumn(
				    "Game", ImGuiTableColumnFlags_WidthFixed,
				    80.0f);
				ImGui::TableSetupColumn(
				    "##Apply", ImGuiTableColumnFlags_WidthFixed,
				    50.0f);
				ImGui::TableSetupColumn(
				    "##Remove",
				    ImGuiTableColumnFlags_WidthFixed, 60.0f);
				...
				for (int i = 0;
				     i < static_cast<int>(mods_.size()); ++i) {
					...
					ImGui::TableNextColumn();
					ImGui::Text(
					    "%s", mod.metadata.version.c_str());

					ImGui::TableNextColumn();
					ImGui::Text(
					    "%s", mod.metadata.author.c_str());

					ImGui::TableNextColumn();
					ImGui::Text("%s",
						    mod.metadata.game.c_str());

					ImGui::TableNextColumn();
					ImGui::PushID(i);
					bool hasGameDir =
					    !gameDirectory_.empty();
					ImGui::BeginDisabled(!hasGameDir);
	...
	if (ImGui::BeginPopupModal("Confirm Apply Mod", nullptr,
				   ImGuiWindowFlags_AlwaysAutoResize)) {
		ImGui::Text("Apply this mod? Files in the game directory will "
			    "be overwritten.");
		ImGui::Separator();
		if (ImGui::Button("Apply", ImVec2(120, 0))) {
			if (pendingModIndex_ >= 0 &&
			    pendingModIndex_ < static_cast<int>(mods_.size())) {
				auto mod = mods_[pendingModIndex_];
				std::string dir = gameDirectory_;
				task_.start([mod, dir](AsyncTask &t) {
					bool ok =
					    ModManager::applyMod(mod, dir, t);
					if (ok) {
...
void ModManagerView::drawCreateModSection() {
	if (ImGui::CollapsingHeader("Create Mod")) {
		...
		ImGui::InputTextMultiline("Description", modDescription_,
					  sizeof(modDescription_),
					  ImVec2(-1, 60));
		...
		if (ImGui::Button("Add File...")) {
			if (auto path = FileDialog::openFile(
				"*", gameDirectory_.c_str())) {
				namespace fs = std::filesystem;
				// Convert absolute path to relative path from
				// game directory.
				auto relPath =
				    fs::relative(*path, gameDirectory_);
		...
		ImGui::Spacing();
		bool canExport = modName_[0] != '\0' &&
				 modVersion_[0] != '\0' && !modFiles_.empty() &&
				 hasGameDir;
		ImGui::BeginDisabled(!canExport);
		if (ImGui::Button("Export Mod (.zip)")) {
			std::string defaultName =
			    std::string(modName_) + ".zip";
			if (auto savePath = FileDialog::saveFile(
				"*.zip", defaultName.c_str())) {
				ModMetadata meta;
				meta.name = modName_;
				...
				task_.start(
				    [meta, dir, files, outPath](AsyncTask &t) {
					    return ModManager::createMod(
						meta, dir, files, outPath, t);
				    });
			}
		}
		ImGui::EndDisabled();
