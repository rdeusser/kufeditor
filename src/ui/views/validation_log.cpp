...
void ValidationLogView::clear() { issues_.clear(); }
...
void ValidationLogView::drawContent() {
	...
	if (ImGui::BeginTable("ValidationTable", 4,
		...
		ImGui::TableSetupColumn(
		    "Record", ImGuiTableColumnFlags_WidthFixed, 60.0f);
		ImGui::TableSetupColumn(
		    "Field", ImGuiTableColumnFlags_WidthFixed, 120.0f);
		...
		for (size_t i = 0; i < issues_.size(); ++i) {
			...
			ImGui::TableNextColumn();
			char label[32];
			snprintf(label, sizeof(label), "#%zu",
				 issue.recordIndex);
			if (ImGui::Selectable(
				label, false,
