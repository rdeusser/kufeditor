...
const char *campaignNames[] = {"Hironeiden (Gerald)", "Vellond (Lucretia)",
			       "Ecclesia (Kendal)", "Dark Legion (Regnier)"};
...
const char *resolveUnitName(const SaveUnit &unit, const NameDictionary &dict) {
	if (unit.leaderNameIndex < 0) {
		const char *name =
		    dict.charInfoName(static_cast<uint8_t>(unit.jobType));
		if (name) return name;
	} else {
		const char *name = dict.leaderGenName(
		    static_cast<uint32_t>(unit.troopInfoIndex),
		    unit.leaderNameIndex);
		if (name) return name;
	}
...
ImVec4 ucdColor(uint32_t ucd) {
	switch (ucd) {
		case 0:
			return ImVec4(0.2f, 0.8f, 0.2f, 1.0f); // Leader — green
		case 1:
			return ImVec4(0.9f, 0.75f, 0.2f,
				      1.0f); // Officer 1 — gold
		case 2:
			return ImVec4(0.9f, 0.75f, 0.2f,
				      1.0f); // Officer 2 — gold
		case 3:
			return ImVec4(0.2f, 0.8f, 0.2f, 1.0f); // Troop — green
	}
...
const char *skillTypeNames[] = {"Melee",    "Range",	 "Frontal",   "Riding",
				"Teamwork", "Scout",	 "Gunpowder", "Taming",
				"Fire",	    "Lightning", "Ice",	      "Holy",
				"Earth",    "Curse",	 "Elemental"};

const char *resistTypeNames[] = {"Melee",  "Ranged",	"Explosion", "Frontal",
				 "Fire",   "Lightning", "Ice",	     "Holy",
				 "Poison", "Curse"};
...
void SaveEditorTab::drawContent() {
	...
	ImGui::BeginChild("SaveSidebar", ImVec2(120, totalHeight),
	...
	switch (currentSection_) {
		case Section::Summary:
			ImGui::BeginChild("SaveSummaryContent",
		...
		case Section::Units:
			ImGui::BeginChild("SaveUnitList",
					  ImVec2(250, totalHeight),
					  ImGuiChildFlags_Borders);
			drawUnitList();
			ImGui::EndChild();
			...
			ImGui::BeginChild("SaveUnitDetails",
					  ImVec2(0, totalHeight),
					  ImGuiChildFlags_Borders);
			if (selectedUnit_ >= 0 &&
			    selectedUnit_ <
				static_cast<int>(
				    document_->saveData->unitCount())) {
		...
		case Section::Roster:
			ImGui::BeginChild("SaveRosterContent",
					  ImVec2(0, totalHeight),
					  ImGuiChildFlags_Borders);
			drawRosterSection();
			ImGui::EndChild();
		...
		case Section::Missions:
			ImGui::BeginChild("SaveMissionsContent",
					  ImVec2(0, totalHeight),
					  ImGuiChildFlags_Borders);
			drawMissionsSection();
			ImGui::EndChild();
...
void SaveEditorTab::drawSidebar() {
	...
	char unitsLabel[32];
	snprintf(unitsLabel, sizeof(unitsLabel), "Units (%zu)",
		 save->unitCount());
	...
	char rosterLabel[32];
	snprintf(rosterLabel, sizeof(rosterLabel), "Roster (%zu)",
		 save->roster().size());
	if (ImGui::Selectable(rosterLabel,
			      currentSection_ == Section::Roster)) {
		currentSection_ = Section::Roster;
	}

	if (ImGui::Selectable("Missions",
...
void SaveEditorTab::drawSummarySection() {
	...
	if (campaign >= 0 && campaign <= 3) {
		if (ComboCentered("Campaign", &campaign, campaignNames, 4)) {
			document_->undoStack->execute(makeSetFieldCommand(
			    &save->campaignIndexRef(),
			    static_cast<int32_t>(campaign), "Change campaign"));
	...
	// Main block fields.
	if (ImGui::CollapsingHeader("Map / File References",
		...
		auto stringInput = [&](const char *label, std::string &str) {
			...
			if (InputTextCentered(label, buf, sizeof(buf))) {
				document_->undoStack->execute(
				    makeSetFieldCommand(&str, std::string(buf),
							std::string("Change ") +
							    label));
				document_->dirty = true;
			}
	...
	if (ImGui::CollapsingHeader("Header Fields")) {
		...
		auto signedFieldInput = [&](const char *label, int32_t &val) {
			...
			if (ImGui::DragInt(label, &val, 1, 0, 0)) {
				...
				val = prev;
				document_->undoStack->execute(
				    makeSetFieldCommand(&val, newVal,
							std::string("Change ") +
							    label));
				document_->dirty = true;
			}
	...
	if (ImGui::DragInt("Selected Unit", &selUnit, 1, -1,
			   static_cast<int>(save->unitCount()) - 1)) {
		document_->undoStack->execute(makeSetFieldCommand(
		    &save->selectedUnitRef(), static_cast<int32_t>(selUnit),
...

	{
		...
		ImGui::Text("%zu units (%d leaders, %d officers1, %d "
			    "officers2, %d troops) "
			    "| Roster: %zu",
			    units.size(), counts[0], counts[1], counts[2],
			    counts[3], save->roster().size());
	}
...
void SaveEditorTab::drawUnitList() {
	...
	for (size_t i = 0; i < units.size(); ++i) {
		...
		// In player-only mode, show leaders, their officers, and
		// standalone troops.
		if (showPlayerOnly_ && !isLeader && !isOfficer && !isTroop)
			continue;
		// Also skip officers whose leader isn't player — walk back to
		// find leader.
		if (showPlayerOnly_ && isOfficer) {
			...
			for (int j = static_cast<int>(i) - 1; j >= 0; --j) {
				...
				if (units[j].ucd != 1 && units[j].ucd != 2)
					break;
			}
		...
		if (isOfficer) {
			...
			if (jtName) {
				snprintf(label, sizeof(label),
					 "[%zu] %s (CharID=%d)", i, jtName,
					 unit.charId);
			} else {
				snprintf(label, sizeof(label),
					 "[%zu] Job %u (CharID=%d)", i,
					 unit.jobType, unit.charId);
			}
		}
		...
		if (ImGui::IsItemHovered()) {
			const char *ucdName =
			    (unit.ucd < 4) ? ucdNames[unit.ucd] : "Unknown";
			ImGui::SetTooltip(
			    "UCD: %s | Lv%u | TroopIdx: %d/%d | Hero: %s",
			    ucdName, unit.skillLevel, unit.troopInfoIndex,
...
void SaveEditorTab::drawUnitDetails(size_t index) {
	...
	if (ImGui::CollapsingHeader("Core", ImGuiTreeNodeFlags_DefaultOpen)) {
		// Job type combo — standard troop types (0-42), then CharInfo
		// entries.
		{
			const char *currentName =
			    resolveUnitName(unit, nameDictionary_);
			char preview[64];
			if (currentName) {
				snprintf(preview, sizeof(preview), "%s (%u)",
					 currentName, unit.jobType);
			} else {
				snprintf(preview, sizeof(preview), "Job %u",
					 unit.jobType);
			}

			if (BeginComboCentered("Job Type", preview)) {
				for (uint32_t i = 0; i <= kMaxStandardJobType;
				     ++i) {
					const char *name =
					    nameDictionary_.troopInfoName(i);
					char itemLabel[64];
					if (name) {
						snprintf(itemLabel,
							 sizeof(itemLabel),
				...
				for (uint32_t i = kMaxStandardJobType + 1;
				     i < 256; ++i) {
					const char *charName =
					    nameDictionary_.charInfoName(
		...
		if (ImGui::DragInt("Model ID", &modelId, 1, 0, 0)) {
			document_->undoStack->execute(makeSetFieldCommand(
			    &unit.modelId,
			    static_cast<uint32_t>(std::max(0, modelId)),
			    "Change model ID"));
			document_->dirty = true;
		}
		...
		if (ImGui::DragInt("Char ID", &charId, 1, 0, 0)) {
			document_->undoStack->execute(makeSetFieldCommand(
			    &unit.charId, static_cast<int32_t>(charId),
			    "Change char ID"));
			document_->dirty = true;
		}
		...
		if (ImGui::DragInt("TroopInfo Index", &troopIdx, 1, 0, 0)) {
			document_->undoStack->execute(
			    makeSetFieldCommand(&unit.troopInfoIndex,
						static_cast<int32_t>(troopIdx),
						"Change troop info index"));
			document_->dirty = true;
		}
		...
		if (ImGui::DragInt("TroopInfo Index 2", &troopIdx2, 1, 0, 0)) {
			document_->undoStack->execute(
			    makeSetFieldCommand(&unit.troopInfoIndex2,
						static_cast<int32_t>(troopIdx2),
						"Change troop info index 2"));
			document_->dirty = true;
		}
		...
		if (ImGui::DragInt("Skill Level", &skillLv, 1, 0, 65535)) {
			document_->undoStack->execute(makeSetFieldCommand(
			    &unit.skillLevel,
			    static_cast<uint32_t>(std::max(0, skillLv)),
			    "Change skill level"));
			document_->dirty = true;
		}
		...
		if (ImGui::Checkbox("Is Hero", &heroFlag)) {
			document_->undoStack->execute(makeSetFieldCommand(
			    &unit.isHero,
			    static_cast<uint8_t>(heroFlag ? 0 : 1),
			    "Toggle hero flag"));
			document_->dirty = true;
		}
		if (ImGui::IsItemHovered())
			ImGui::SetTooltip("0 = Hero, non-zero = Troop");

		int b58 = unit.byte58;
		if (ImGui::DragInt("Byte 0x58", &b58, 1, 0, 255)) {
			document_->undoStack->execute(makeSetFieldCommand(
			    &unit.byte58,
			    static_cast<uint8_t>(std::clamp(b58, 0, 255)),
			    "Change byte 0x58"));
			document_->dirty = true;
		}
		...
		if (ImGui::DragInt("Byte 0x5A", &b5a, 1, 0, 255)) {
			document_->undoStack->execute(makeSetFieldCommand(
			    &unit.byte5a,
			    static_cast<uint8_t>(std::clamp(b5a, 0, 255)),
			    "Change byte 0x5A"));
			document_->dirty = true;
		}
	}
	...
	if (ImGui::CollapsingHeader("Formation")) {
		...
		if (ImGui::DragInt("Grid Config", &gridCfg, 1, 0, 0)) {
			document_->undoStack->execute(makeSetFieldCommand(
			    &unit.gridConfig,
			    static_cast<uint32_t>(std::max(0, gridCfg)),
			    "Change grid config"));
			document_->dirty = true;
		}
	}
	...
	if (ImGui::CollapsingHeader("Equipment",
		...
		for (int e = 0; e < 6; ++e) {
			ImGui::PushID(e);
			drawEquipmentSlot(equipSlotLabels[e],
					  unit.equipSlots()[e], e);
			ImGui::PopID();
		}
	}
	...
	if (ImGui::CollapsingHeader("Unknown Fields")) {
		...
		if (ImGui::DragInt("Leader Name Index", &nameIdx, 1, 0, 0)) {
			document_->undoStack->execute(
			    makeSetFieldCommand(&unit.leaderNameIndex,
						static_cast<int32_t>(nameIdx),
						"Change leader name index"));
			document_->dirty = true;
		}
...
void SaveEditorTab::drawEquipmentSlot(const char *label,
				      SaveEquipmentSlot &slot, int slotIdx) {
	...
	if (isEmpty) {
		snprintf(headerLabel, sizeof(headerLabel),
			 "%s (Empty)###equip%d", label, slotIdx);
	} else {
		std::string name = nameDictionary_.weaponName(
		    slot.itemTypeId, slot.variantIndex, slot.enhancementTier);
		if (name.empty()) {
			snprintf(headerLabel, sizeof(headerLabel),
				 "%s: Item %d, Lv%u###equip%d", label,
				 slot.itemTypeId, slot.level, slotIdx);
		} else {
			// Add attribute suffix.
			std::string suffix;
			const char *att1 =
			    nameDictionary_.itemAttName(slot.attribute1Index);
			const char *att2 =
			    nameDictionary_.itemAttName(slot.attribute2Index);
			if (att1 && att2) {
				suffix =
				    std::string(" of ") + att1 + " and " + att2;
			} else if (att1) {
			...
			snprintf(headerLabel, sizeof(headerLabel),
				 "%s: %s%s, Lv%u###equip%d", label,
				 name.c_str(), suffix.c_str(), slot.level,
				 slotIdx);
		}
	}
	...
	const char *desc1 =
	    nameDictionary_.itemAttDescription(slot.attribute1Index);
	const char *desc2 =
	    nameDictionary_.itemAttDescription(slot.attribute2Index);
	if (desc1 || desc2) {
	...
	// Core editable fields.
	{
		...
		if (typeCount > 0) {
			std::string currentName =
			    nameDictionary_.itemTypeBaseName(slot.itemTypeId);
			char preview[64];
			if (!currentName.empty()) {
				snprintf(preview, sizeof(preview), "%s (%d)",
					 currentName.c_str(), slot.itemTypeId);
			} else {
				snprintf(preview, sizeof(preview), "Item %d",
					 slot.itemTypeId);
			}

			if (BeginComboCentered("Item Type", preview)) {
				for (int t = 0; t < static_cast<int>(typeCount);
				     ++t) {
					std::string name =
					    nameDictionary_.itemTypeBaseName(t);
		...
		if (ImGui::DragInt("Level", &level, 1, 0, 65535)) {
			document_->undoStack->execute(makeSetFieldCommand(
			    &slot.level,
			    static_cast<uint16_t>(std::max(0, level)),
			    "Change level"));
			document_->dirty = true;
		}
		...
		if (ImGui::DragInt("Enhancement Tier", &tier, 1, -1, 2)) {
			document_->undoStack->execute(makeSetFieldCommand(
			    &slot.enhancementTier, static_cast<int16_t>(tier),
			    "Change enhancement tier"));
			document_->dirty = true;
		}
		...
		if (ImGui::DragInt("Variant Index", &variant, 1, 0, 65535)) {
			document_->undoStack->execute(makeSetFieldCommand(
			    &slot.variantIndex,
			    static_cast<uint16_t>(std::max(0, variant)),
	...
	{
		auto skillCombo = [&](const char *comboLabel,
				      const char *bonusLabel, int32_t &type,
				      int32_t &bonus) {
			const char *preview = (type >= 0 && type <= 14)
						  ? skillTypeNames[type]
						  : "(None)";
			if (BeginComboCentered(comboLabel, preview)) {
				...
				for (int s = 0; s <= 14; ++s) {
					bool sel = (type == s);
					if (ImGui::Selectable(skillTypeNames[s],
							      sel)) {
						document_->undoStack->execute(
						    makeSetFieldCommand(
							&type,
			...
			if (ImGui::DragInt(bonusLabel, &b)) {
				document_->undoStack->execute(
				    makeSetFieldCommand(
					&bonus, static_cast<int32_t>(b),
					std::string("Change ") + bonusLabel));
				document_->dirty = true;
			}
		};

		skillCombo("Skill Type 1", "Skill Bonus 1", slot.skillType1,
			   slot.skillBonus1);
		skillCombo("Skill Type 2", "Skill Bonus 2", slot.skillType2,
			   slot.skillBonus2);
	}

	// Resist type combos with bonus inputs.
	{
		auto resistCombo = [&](const char *comboLabel,
				       const char *bonusLabel, int32_t &type,
				       int32_t &bonus) {
			const char *preview = (type >= 0 && type <= 9)
						  ? resistTypeNames[type]
						  : "(None)";
			if (BeginComboCentered(comboLabel, preview)) {
				bool noneSelected = (type < 0);
				if (ImGui::Selectable("(None)", noneSelected)) {
		...
		resistCombo("Resist Type 1", "Resist Bonus 1", slot.resistType1,
			    slot.resistBonus1);
		resistCombo("Resist Type 2", "Resist Bonus 2", slot.resistType2,
			    slot.resistBonus2);
	}

	// Attribute dropdowns (fall back to DragInt if no attribute data
	// loaded).
	{
		auto attCombo = [&](const char *label, int32_t &attrIndex) {
			...
			if (attCount > 0) {
				const char *currentName =
				    nameDictionary_.itemAttName(attrIndex);
				char preview[64];
				...
				if (BeginComboCentered(label, preview)) {
					...
					for (int a = 0;
					     a < static_cast<int>(attCount);
					     ++a) {
						const char *name =
						    nameDictionary_.itemAttName(
							a);
						if (!name) continue;
						char itemLabel[64];
						snprintf(itemLabel,
	...
	if (ImGui::TreeNode("Other Fields")) {
		...
		if (ImGui::DragInt("Auto ID", &autoId)) {
			document_->undoStack->execute(makeSetFieldCommand(
			    &slot.autoId, static_cast<uint32_t>(autoId),
			    "Change auto ID"));
			document_->dirty = true;
		}
		...
		if (ImGui::DragInt("Item Power", &power)) {
			document_->undoStack->execute(makeSetFieldCommand(
			    &slot.itemPower, static_cast<int16_t>(power),
			    "Change item power"));
			document_->dirty = true;
		}
		...
		if (ImGui::DragInt("Equipped Flag", &equipped, 1, 0, 65535)) {
			document_->undoStack->execute(makeSetFieldCommand(
			    &slot.equippedFlag,
			    static_cast<uint16_t>(std::max(0, equipped)),
			    "Change equipped flag"));
			document_->dirty = true;
		}
		...
		if (ImGui::DragInt("Reserved", &reserved, 1, 0, 65535)) {
			document_->undoStack->execute(makeSetFieldCommand(
			    &slot.reserved,
			    static_cast<uint16_t>(std::max(0, reserved)),
			    "Change reserved"));
			document_->dirty = true;
		}
		...
		if (ImGui::DragInt("Slot Category", &cat)) {
			document_->undoStack->execute(makeSetFieldCommand(
			    &slot.slotCategory, static_cast<int32_t>(cat),
			    "Change slot category"));
			document_->dirty = true;
		}
...
void SaveEditorTab::drawRosterSection() {
	...
	if (ImGui::BeginTable("PlayerUnitsTable", 5,
		...
		ImGui::TableSetupColumn(
		    "Level", ImGuiTableColumnFlags_WidthFixed, 50.0f);
		ImGui::TableSetupColumn(
		    "CharID", ImGuiTableColumnFlags_WidthFixed, 60.0f);
		ImGui::TableSetupColumn(
		    "Hero", ImGuiTableColumnFlags_WidthFixed, 40.0f);
	...
	if (ImGui::BeginTable("RosterTable", 6,
		...
		ImGui::TableSetupColumn("#", ImGuiTableColumnFlags_WidthFixed,
					40.0f);
		ImGui::TableSetupColumn(
		    "Byte 60", ImGuiTableColumnFlags_WidthFixed, 80.0f);
		ImGui::TableSetupColumn(
		    "Byte 61", ImGuiTableColumnFlags_WidthFixed, 80.0f);
		ImGui::TableSetupColumn(
		    "Byte 62", ImGuiTableColumnFlags_WidthFixed, 80.0f);
		ImGui::TableSetupColumn(
		    "Byte 63", ImGuiTableColumnFlags_WidthFixed, 80.0f);
		ImGui::TableSetupColumn("Value 64",
					ImGuiTableColumnFlags_WidthStretch);
		ImGui::TableHeadersRow();
...
void SaveEditorTab::drawMissionsSection() {
	...
	if (ImGui::DragInt("Current Mission Index", &missionIdx, 1, 0, 0)) {
		document_->undoStack->execute(
		    makeSetFieldCommand(&save->currentMissionIndexRef(),
					static_cast<int32_t>(missionIdx),
					"Change current mission index"));
		document_->dirty = true;
	}
	...
	if (ImGui::BeginTable("MissionTable", 4,
				  ...
				  ImGuiTableFlags_RowBg)) {
		ImGui::TableSetupColumn(
		    "Slot", ImGuiTableColumnFlags_WidthFixed, 60.0f);
		...
		for (size_t i = 0; i < kSaveMissionSlots; i += 2) {
			...
			ImGui::SetNextItemWidth(-1);
			{
				...
				if (ImGui::DragInt("##mc", &completion[i], 1, 0,
						   0)) {
