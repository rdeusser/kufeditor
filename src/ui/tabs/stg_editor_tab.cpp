...
// 0x005597a0).
constexpr uint8_t kCharInfoJobTypes[] = {32, 33, 34, 35, 36, 37,
					 38, 43, 44, 46, 47};
...
bool asciiPrefixMatch(const std::vector<std::byte> &key,
...
std::string resolveSpecialName(const std::string &unitName,
...
std::string resolveDisplayName(const StgUnit &unit,
			       const NameDictionary &dict) {
...
void StgEditorTab::drawContent() {
	...
	if (currentSection_ == Section::Header) {
		...
		if (!document_->stgData->tailParsed()) {
			...
			ImGui::BeginChild("StgEventDetails",
					  ImVec2(0, totalHeight),
					  ImGuiChildFlags_Borders);
			if (selectedBlock_ >= 0 &&
			    selectedBlock_ <
				static_cast<int>(
				    document_->stgData->eventBlocks().size()) &&
			    selectedEvent_ >= 0 &&
			    selectedEvent_ <
				static_cast<int>(
				    document_->stgData
					->eventBlocks()[selectedBlock_]
					.events.size())) {
				drawEventDetails(selectedBlock_,
						 selectedEvent_);
			} else {
...
void StgEditorTab::drawEventList() {
	...
	if (ImGui::SmallButton("+ Add Event")) {
		...
		blocks[selectedBlock_ >= 0 ? selectedBlock_ : 0]
		    .events.push_back(newEvent);
		selectedBlock_ = selectedBlock_ >= 0 ? selectedBlock_ : 0;
		selectedEvent_ =
		    static_cast<int>(blocks[selectedBlock_].events.size() - 1);
		document_->dirty = true;
	}
	...
	for (size_t b = 0; b < blocks.size(); ++b) {
		...
		snprintf(blockLabel, sizeof(blockLabel),
			 "Block %zu (%zu events)", b, block.events.size());

		if (ImGui::TreeNodeEx(blockLabel,
				      ImGuiTreeNodeFlags_DefaultOpen)) {
			int deleteIndex = -1;
			...
			for (size_t i = 0; i < block.events.size(); ++i) {
				...
				std::string displayEventDesc =
				    nameDictionary_.translate(
					event.description);
				if (displayEventDesc.empty())
					displayEventDesc = event.description;
			...
			if (deleteIndex >= 0) {
				block.events.erase(block.events.begin() +
						   deleteIndex);
				if (selectedBlock_ == static_cast<int>(b) &&
				    selectedEvent_ >=
					static_cast<int>(block.events.size())) {
					selectedEvent_ =
					    static_cast<int>(
						block.events.size()) -
					    1;
...
void StgEditorTab::drawEventDetails(size_t blockIdx, size_t eventIdx) {
	auto &event =
	    document_->stgData->eventBlocks()[blockIdx].events[eventIdx];

	ImGui::Text("Block %zu / Event %zu", blockIdx, eventIdx);
	ImGui::Separator();

	if (ImGui::CollapsingHeader("Event Header",
				    ImGuiTreeNodeFlags_DefaultOpen)) {
		std::string translatedEventDesc =
		    nameDictionary_.translate(event.description);
		const std::string &displayDesc = translatedEventDesc.empty()
						     ? event.description
						     : translatedEventDesc;
		...
		std::memset(descBuf, 0, sizeof(descBuf));
		std::strncpy(descBuf, displayDesc.c_str(), sizeof(descBuf) - 1);
		if (InputTextCentered("Description", descBuf,
				      sizeof(descBuf))) {
			std::string input(descBuf);
			std::string korean =
			    nameDictionary_.reverseTranslate(input);
		...
		if (ImGui::DragInt("Event ID", &eventId, 1, 0, 0)) {
			event.eventId =
			    static_cast<uint32_t>(std::max(0, eventId));
			event.modified = true;
			document_->dirty = true;
		}
	}
	...
	if (ImGui::CollapsingHeader("Conditions",
		...
		for (size_t i = 0; i < event.conditions.size(); ++i) {
			...
			char entryLabel[32];
			snprintf(entryLabel, sizeof(entryLabel),
				 "Condition %zu", i);
			drawScriptEntry(entryLabel, event.conditions[i], true,
					event, "COND_REORDER",
					static_cast<int>(i), &condDragSrc,
					&condDragDst);
		...
		if (condDragSrc >= 0 && condDragDst >= 0 &&
		    condDragSrc != condDragDst) {
			document_->undoStack->execute(
			    std::make_unique<
				ReorderVectorCommand<StgScriptEntry>>(
				&event.conditions, condDragSrc, condDragDst,
				"Reorder condition"));
			event.modified = true;
		}
		if (condDelete >= 0) {
			event.conditions.erase(event.conditions.begin() +
					       condDelete);
			event.modified = true;
			document_->dirty = true;
		}
	...
	if (ImGui::CollapsingHeader("Actions",
		...
		for (size_t i = 0; i < event.actions.size(); ++i) {
			...
			char entryLabel[32];
			snprintf(entryLabel, sizeof(entryLabel), "Action %zu",
				 i);
			drawScriptEntry(entryLabel, event.actions[i], false,
					event, "ACT_REORDER",
					static_cast<int>(i), &actDragSrc,
					&actDragDst);
...
void StgEditorTab::drawScriptEntry(const char *entryLabel,
				   StgScriptEntry &entry, bool isCondition,
				   StgEvent &event, const char *dragPayloadType,
				   int dragIndex, int *dragSrc, int *dragDst) {
	...
	bool open = ImGui::TreeNode(entryLabel, "%s: %s (%u)", entryLabel, name,
				    entry.typeId);
	...
	if (dragPayloadType) {
		if (ImGui::BeginDragDropSource(ImGuiDragDropFlags_None)) {
			ImGui::SetDragDropPayload(dragPayloadType, &dragIndex,
						  sizeof(int));
	...
	if (open) {
		...
		const ScriptEntryInfo *catalog =
		    isCondition ? kConditions : kActions;
		size_t catalogSize =
		    isCondition ? kConditionCount : kActionCount;

		char preview[64];
		if (info) {
			snprintf(preview, sizeof(preview), "%s (%u)",
				 info->name, entry.typeId);
		} else {
			snprintf(preview, sizeof(preview), "Unknown (%u)",
				 entry.typeId);
		}
		...
		info = isCondition ? findConditionInfo(entry.typeId)
				   : findActionInfo(entry.typeId);
		...
		for (size_t i = 0; i < entry.params.size(); ++i) {
			...
			char labelBuf[64];
			snprintf(labelBuf, sizeof(labelBuf), "%s [%zu]",
				 paramHint ? paramHint : "Param", i);
...
void StgEditorTab::drawParamValue(const char *label, StgParamValue &param,
