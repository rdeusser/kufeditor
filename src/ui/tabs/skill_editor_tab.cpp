...
void SkillEditorTab::drawContent() {
	...
	ImGui::BeginChild("SkillList", ImVec2(250, listHeight),
	...
	ImGui::BeginChild("SkillDetails", ImVec2(0, listHeight),
			  ImGuiChildFlags_Borders);
	if (selectedSkill_ >= 0 &&
	    selectedSkill_ <
		static_cast<int>(document_->skillData->skills().size())) {
...
void SkillEditorTab::drawSkillList() {
	...
	for (size_t i = 0; i < skills.size(); ++i) {
		const char *name =
		    (i < std::size(SKILL_NAMES)) ? SKILL_NAMES[i] : "Unknown";
...
void SkillEditorTab::drawSkillDetails(size_t index) {
	...
	const char *name =
	    (index < std::size(SKILL_NAMES)) ? SKILL_NAMES[index] : "Unknown";
	...
	if (ImGui::DragInt("Skill Type", &skillType, 1, 1, 2)) {
		skill.skillType =
		    static_cast<uint32_t>(std::clamp(skillType, 1, 2));
		document_->dirty = true;
	}
	...
	if (ImGui::DragInt("Max Level", &maxLevel, 1, 1, 65535)) {
		skill.maxLevel =
		    static_cast<uint32_t>(std::clamp(maxLevel, 1, 65535));
		document_->dirty = true;
	}
