...
class StgEditorTab : public EditorTab {
	...
	const NameDictionary &nameDictionary() const { return nameDictionary_; }
      ...
      private:
	enum class Section { Header, Units, Areas, Variables, Events };

	void drawSidebar();
	void drawHeaderSection();
	void drawUnitList();
	void drawUnitDetails(size_t index);
	void drawOfficerSection(const char *label, OfficerData &officer,
				bool active);
	void drawAreaList();
	...
	void drawScriptEntry(const char *entryLabel, StgScriptEntry &entry,
	...
	void drawParamValue(const char *label, StgParamValue &param,
			    StgEvent &event, const char *paramHint = nullptr);
