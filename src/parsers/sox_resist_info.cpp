...
ResistInfoRecord ResistInfoRecord::parse(const uint8_t* buf, size_t len, size_t& offset) {
    ResistInfoRecord result;
    if (offset + 4 > len) throw std::runtime_error("buffer overflow");
    std::memcpy(&result.resist_id, buf + offset, 4);
    offset += 4;
    if (offset + 4 > len) throw std::runtime_error("buffer overflow");
    std::memcpy(&result.unknown, buf + offset, 4);
    offset += 4;
...
std::string ResistInfoRecord::to_json() const {
    std::string s = "{";
    s += "\"resist_id\":";
    s += std::to_string(resist_id);
    s += ",";
    s += "\"unknown\":";
...
std::vector<uint8_t> ResistInfoRecord::to_bytes() const {
    std::vector<uint8_t> _buf;
    {
        uint32_t _tmp = resist_id;
        uint8_t _raw[4];
        std::memcpy(_raw, &_tmp, 4);
        _buf.insert(_buf.end(), _raw, _raw + 4);
    }
    {
        uint32_t _tmp = unknown;
        uint8_t _raw[4];
        std::memcpy(_raw, &_tmp, 4);
        _buf.insert(_buf.end(), _raw, _raw + 4);
    }
...
File File::parse(const uint8_t* buf, size_t len, size_t& offset) {
    ...
    for (size_t i = 0; i < result.header.record_count; ++i) {
        result.records.push_back(ResistInfoRecord::parse(buf, len, offset));
    }
