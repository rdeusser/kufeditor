...
#include "sox_skill_point_table.h"
...
SkillPointRecord SkillPointRecord::parse(const uint8_t* buf, size_t len, size_t& offset) {
    SkillPointRecord result;
    if (offset + 4 > len) throw std::runtime_error("buffer overflow");
    std::memcpy(&result.level, buf + offset, 4);
    offset += 4;
    if (offset + 4 > len) throw std::runtime_error("buffer overflow");
    std::memcpy(&result.cumulative_xp, buf + offset, 4);
    offset += 4;
    return result;
}

std::string SkillPointRecord::to_json() const {
    std::string s = "{";
    s += "\"level\":";
    s += std::to_string(level);
    s += ",";
    s += "\"cumulative_xp\":";
    s += std::to_string(cumulative_xp);
    s += "}";
    return s;
}

std::vector<uint8_t> SkillPointRecord::to_bytes() const {
    std::vector<uint8_t> _buf;
    {
        uint32_t _tmp = level;
        uint8_t _raw[4];
...
File File::parse(const uint8_t* buf, size_t len, size_t& offset) {
    ...
    for (size_t i = 0; i < result.header.record_count; ++i) {
        result.records.push_back(SkillPointRecord::parse(buf, len, offset));
    }
