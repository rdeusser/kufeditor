...
std::vector<std::string> readJsonStringArray(const std::string &text,
					     size_t &pos) {
...
std::optional<ModMetadata> parseModJson(const std::string &text) {
	...
	if (meta.name.empty() || meta.version.empty() || meta.game.empty() ||
	    meta.files.empty()) {
		return std::nullopt;
	}
...
std::string serializeModJson(const ModMetadata &meta) {
	...
	if (!meta.description.empty()) {
		out << "  \"description\": \""
		    << escapeJsonString(meta.description) << "\",\n";
	}
...
std::string
serializeInstalledModsJson(const std::vector<InstalledModInfo> &mods) {
	std::ostringstream out;
	out << "{\n  \"mods\": [\n";
	for (size_t i = 0; i < mods.size(); ++i) {
		...
		out << "      \"game\": \"" << escapeJsonString(m.game)
		    << "\",\n";
		out << "      \"installedAt\": \""
		    << escapeJsonString(m.installedAt) << "\",\n";
