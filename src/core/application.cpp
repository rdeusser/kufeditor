...
Application::Application() {
	...
	tabManager_->setOnDocumentOpened([this](OpenDocument *doc) {
		if (doc && !doc->path.empty()) {
			...
			settingsDialog_->config().recentFiles =
			    recentFiles_->files();
...
void Application::drawMenuBar() {
	if (ImGui::BeginMainMenuBar()) {
		if (ImGui::BeginMenu("File")) {
			...
			auto *activeTab = tabManager_->activeTab();
			bool hasFile = activeTab && activeTab->document() &&
				       activeTab->document()->hasData();
		...
		if (ImGui::BeginMenu("Edit")) {
			auto *activeTab = tabManager_->activeTab();
			auto *activeDoc =
			    activeTab ? activeTab->document().get() : nullptr;
			auto *undoStack =
			    activeDoc ? activeDoc->undoStack.get() : nullptr;
...
#endif
			if (ImGui::MenuItem(undoLabel.c_str(), kUndoShortcut,
					    false, canUndo)) {
				undoStack->undo();
			}
			if (ImGui::MenuItem(redoLabel.c_str(), kRedoShortcut,
					    false, canRedo)) {
				undoStack->redo();
			}
			ImGui::Separator();
...
void Application::drawTabBar() {
	...
	if (ImGui::BeginTabBar("MainTabBar", tabBarFlags)) {
		...
		for (const auto &tab : tabManager_->tabs()) {
			...
			if (ImGui::BeginTabItem(tab->tabTitle().c_str(), &open,
						flags)) {
...
void Application::drawDockspace() {
	...
	if (activeTab && activeTab->document()) {
		...
		if (doc->binaryData) {
			ImGui::Text("%s%s | %s | %zu troops", doc->path.c_str(),
				    doc->dirty ? "*" : "",
				    doc->binaryData->detectedVersion() ==
					    GameVersion::Crusaders
					? "Crusaders"
					: "Heroes",
				    doc->binaryData->recordCount());
		} else if (doc->textData) {
			ImGui::Text("%s%s | Text SOX | %zu entries",
				    doc->path.c_str(), doc->dirty ? "*" : "",
			...
			static const char *sCampaigns[] = {
			    "Hironeiden", "Vellond", "Ecclesia", "Dark Legion"};
			int ci = doc->saveData->campaignIndex();
			const char *cname =
			    (ci >= 0 && ci <= 3) ? sCampaigns[ci] : "Unknown";
			ImGui::Text("%s%s | Save Game | %s | %zu units",
				    doc->path.c_str(), doc->dirty ? "*" : "",
				    cname, doc->saveData->unitCount());
		} else {
