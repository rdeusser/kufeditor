...
ZipReader::readEntry(const std::string &name) const {
...
bool ZipReader::extractEntry(const std::string &name,
	...
	return mz_zip_reader_extract_to_file(zip, static_cast<mz_uint>(index),
					     destPath.c_str(), 0);
}
...
bool ZipWriter::addFile(const std::string &diskPath,
...
bool ZipWriter::addMemory(const std::string &archiveName, const void *data,
			  size_t size) {
	if (!archive_ || finalized_) return false;
	auto *zip = static_cast<mz_zip_archive *>(archive_);
	return mz_zip_writer_add_mem(zip, archiveName.c_str(), data, size,
