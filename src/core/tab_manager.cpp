...
std::shared_ptr<OpenDocument>
TabManager::loadDocument(const std::string &path) {
	...
	if (ext == ".sav") {
		...
		if (save->load(doc->rawData)) {
			doc->saveData = save;
			doc->undoStack->setOnChange(
			    [doc = doc.get()]() { doc->dirty = true; });
	...
	// SOX files are normally pure binary. Decode if hex-encoded
	// (non-standard).
