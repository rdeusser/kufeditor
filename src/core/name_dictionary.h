...
class NameDictionary {
	...
	const std::vector<SpecialNameEntry> &specialNames() const {
		return specialNames_;
	}

	std::string weaponName(int32_t itemTypeId, uint16_t variantIndex,
	...
	bool loaded() const { return loaded_; }
      ...
      private:
	bool loadIndexedTextSox(const std::string &path,
				std::vector<std::string> &entries);
	bool loadSpecialNamesSox(const std::string &soxPath,
