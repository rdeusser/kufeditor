...
#[allow(non_snake_case, reason = "schema field names are preserved")]
#[derive(Debug, Clone, PartialEq)]
pub struct SkillPointRecord {
    pub level: u32,
    pub cumulative_xp: u32,
}

impl SkillPointRecord {
    #[allow(non_snake_case, reason = "schema field names are preserved")]
    #[allow(clippy::too_many_lines, reason = "schema methods follow field order")]
    pub fn parse(buf: &[u8], offset: &mut usize) -> ::std::result::Result<Self, Error> {
        let level = u32::from_le_bytes(self::read_array::<4>(buf, offset)?);
        let cumulative_xp = u32::from_le_bytes(self::read_array::<4>(buf, offset)?);
        Ok(Self {
            level,
            cumulative_xp,
        })
...
impl SkillPointRecord {
    ...
    pub fn to_json(&self) -> ::std::string::String {
        ...
        json.push_str(&value.to_string());
        json.push(',');
        write_json_string(&mut json, "cumulative_xp");
        json.push(':');
        let value = &self.cumulative_xp;
        json.push_str(&value.to_string());
        json.push('}');
...
pub struct File {
    ...
    pub records: ::std::vec::Vec<SkillPointRecord>,
    pub footer: [u8; 64],
}
...
impl File {
    ...
    pub fn parse(buf: &[u8], offset: &mut usize) -> ::std::result::Result<Self, Error> {
        ...
        let records = {
            ...
            for _ in 0..count {
                values.push(SkillPointRecord::parse(buf, offset)?);
            }
            values
        };
        let footer = {
