...
#[allow(non_snake_case, reason = "schema field names are preserved")]
#[derive(Debug, Clone, PartialEq)]
pub struct UnitUvidRecord {
    pub unit_id: i32,
    pub upgrade_slots: [i32; 17],
}
...
impl UnitUvidRecord {
    ...
    #[allow(clippy::too_many_lines, reason = "schema methods follow field order")]
    pub fn parse(buf: &[u8], offset: &mut usize) -> ::std::result::Result<Self, Error> {
        let unit_id = i32::from_le_bytes(self::read_array::<4>(buf, offset)?);
        let upgrade_slots = {
            let mut values = ::std::vec::Vec::with_capacity(17);
            for _ in 0..17 {
                values.push(i32::from_le_bytes(self::read_array::<4>(buf, offset)?));
            }
            <[_; 17]>::try_from(values).map_err(|values| Error::FixedSize {
                field: "upgrade_slots",
                expected: 17,
                actual: values.len(),
            })?
...
impl UnitUvidRecord {
    ...
    pub fn to_bytes(&self) -> ::std::result::Result<::std::vec::Vec<u8>, Error> {
        let mut buf = ::std::vec::Vec::new();
        let value = &self.unit_id;
        buf.extend_from_slice(&(*value).to_le_bytes());
        let value = &self.upgrade_slots;
        for item in value {
...
impl UnitUvidRecord {
    ...
    pub fn to_json(&self) -> ::std::string::String {
        let mut json = ::std::string::String::from("{");
        write_json_string(&mut json, "unit_id");
        json.push(':');
        let value = &self.unit_id;
        json.push_str(&value.to_string());
        json.push(',');
        write_json_string(&mut json, "upgrade_slots");
        json.push(':');
        let value = &self.upgrade_slots;
        json.push('[');
...
pub struct File {
    ...
    pub records: ::std::vec::Vec<UnitUvidRecord>,
    pub footer: [u8; 64],
}
...
impl File {
    ...
    pub fn parse(buf: &[u8], offset: &mut usize) -> ::std::result::Result<Self, Error> {
        ...
        let records = {
            ...
            for _ in 0..count {
                values.push(UnitUvidRecord::parse(buf, offset)?);
            }
            values
        };
        let footer = {
