...
impl ::std::convert::TryFrom<i32> for SkillType {
    type Error = Error;
...
impl ::std::convert::From<SkillType> for i32 {
...
impl SkillInfoRecord {
    #[allow(non_snake_case, reason = "schema field names are preserved")]
    #[allow(clippy::too_many_lines, reason = "schema methods follow field order")]
    pub fn parse(buf: &[u8], offset: &mut usize) -> ::std::result::Result<Self, Error> {
        let skill_id = i32::from_le_bytes(self::read_array::<4>(buf, offset)?);
        let loc_key = Lps::parse(buf, offset)?;
        let icon = Lps::parse(buf, offset)?;
        let skill_type = u32::from_le_bytes(self::read_array::<4>(buf, offset)?);
        let max_level = u32::from_le_bytes(self::read_array::<4>(buf, offset)?);
        Ok(Self {
            skill_id,
            loc_key,
            icon,
...
impl SkillInfoRecord {
    ...
    pub fn to_bytes(&self) -> ::std::result::Result<::std::vec::Vec<u8>, Error> {
        let mut buf = ::std::vec::Vec::new();
        let value = &self.skill_id;
        buf.extend_from_slice(&(*value).to_le_bytes());
        let value = &self.loc_key;
        buf.extend_from_slice(&value.to_bytes()?);
        let value = &self.icon;
        buf.extend_from_slice(&value.to_bytes()?);
        let value = &self.skill_type;
        buf.extend_from_slice(&(*value).to_le_bytes());
        let value = &self.max_level;
        buf.extend_from_slice(&(*value).to_le_bytes());
        Ok(buf)
    }
}
...
impl SkillInfoRecord {
    ...
    pub fn to_json(&self) -> ::std::string::String {
        let mut json = ::std::string::String::from("{");
        write_json_string(&mut json, "skill_id");
        json.push(':');
        let value = &self.skill_id;
        json.push_str(&value.to_string());
        json.push(',');
        ...
        json.push_str(&value.to_json());
        json.push(',');
        write_json_string(&mut json, "skill_type");
        json.push(':');
        let value = &self.skill_type;
        json.push_str(&value.to_string());
        json.push(',');
        write_json_string(&mut json, "max_level");
        json.push(':');
        let value = &self.max_level;
        json.push_str(&value.to_string());
        json.push('}');
...
pub struct File {
    ...
    pub records: ::std::vec::Vec<SkillInfoRecord>,
    pub footer: [u8; 64],
}
...
impl File {
    ...
    pub fn parse(buf: &[u8], offset: &mut usize) -> ::std::result::Result<Self, Error> {
        ...
        let records = {
            ...
            for _ in 0..count {
                values.push(SkillInfoRecord::parse(buf, offset)?);
            }
            values
        };
        let footer = {
