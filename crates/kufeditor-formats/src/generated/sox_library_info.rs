...
#[allow(non_snake_case, reason = "schema field names are preserved")]
#[derive(Debug, Clone, PartialEq)]
pub struct LibraryInfoRecord {
    pub data_offset: u32,
    pub unknown: u16,
}

impl LibraryInfoRecord {
    #[allow(non_snake_case, reason = "schema field names are preserved")]
    #[allow(clippy::too_many_lines, reason = "schema methods follow field order")]
    pub fn parse(buf: &[u8], offset: &mut usize) -> ::std::result::Result<Self, Error> {
        let data_offset = u32::from_le_bytes(self::read_array::<4>(buf, offset)?);
        let unknown = u16::from_le_bytes(self::read_array::<2>(buf, offset)?);
        Ok(Self {
            data_offset,
            unknown,
        })
...
impl LibraryInfoRecord {
    ...
    pub fn to_json(&self) -> ::std::string::String {
        let mut json = ::std::string::String::from("{");
        write_json_string(&mut json, "data_offset");
        json.push(':');
        let value = &self.data_offset;
        json.push_str(&value.to_string());
        json.push(',');
...
pub struct File {
    ...
    pub records: ::std::vec::Vec<LibraryInfoRecord>,
    pub footer: [u8; 64],
}
...
impl File {
    ...
    pub fn parse(buf: &[u8], offset: &mut usize) -> ::std::result::Result<Self, Error> {
        ...
        let records = {
            ...
            for _ in 0..count {
                values.push(LibraryInfoRecord::parse(buf, offset)?);
            }
            values
        };
        let footer = {
