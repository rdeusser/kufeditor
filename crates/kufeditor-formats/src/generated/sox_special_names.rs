...
#[allow(non_snake_case, reason = "schema field names are preserved")]
#[derive(Debug, Clone, PartialEq)]
pub struct SpecialNamesRecord {
    pub key: LpsBytes,
    pub value: LpsBytes,
}

impl SpecialNamesRecord {
    #[allow(non_snake_case, reason = "schema field names are preserved")]
    #[allow(clippy::too_many_lines, reason = "schema methods follow field order")]
    pub fn parse(buf: &[u8], offset: &mut usize) -> ::std::result::Result<Self, Error> {
        let key = LpsBytes::parse(buf, offset)?;
        let value = LpsBytes::parse(buf, offset)?;
        Ok(Self { key, value })
    }
}
...
pub struct File {
    ...
    pub records: ::std::vec::Vec<SpecialNamesRecord>,
    pub footer: [u8; 64],
}
...
impl File {
    ...
    pub fn parse(buf: &[u8], offset: &mut usize) -> ::std::result::Result<Self, Error> {
        ...
        let records = {
            ...
            for _ in 0..count {
                values.push(SpecialNamesRecord::parse(buf, offset)?);
            }
            values
        };
        let footer = {
