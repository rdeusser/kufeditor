...
impl ::std::convert::TryFrom<i32> for ResistType {
    type Error = Error;
...
impl ::std::convert::From<ResistType> for i32 {
    fn from(value: ResistType) -> Self {
        value as i32
    }
}
...
impl ResistInfoRecord {
    #[allow(non_snake_case, reason = "schema field names are preserved")]
    #[allow(clippy::too_many_lines, reason = "schema methods follow field order")]
    pub fn parse(buf: &[u8], offset: &mut usize) -> ::std::result::Result<Self, Error> {
        let resist_id = u32::from_le_bytes(self::read_array::<4>(buf, offset)?);
        let unknown = u32::from_le_bytes(self::read_array::<4>(buf, offset)?);
...
impl ResistInfoRecord {
    ...
    pub fn to_json(&self) -> ::std::string::String {
        let mut json = ::std::string::String::from("{");
        write_json_string(&mut json, "resist_id");
        json.push(':');
        let value = &self.resist_id;
        json.push_str(&value.to_string());
        json.push(',');
        write_json_string(&mut json, "unknown");
        json.push(':');
        let value = &self.unknown;
        json.push_str(&value.to_string());
        json.push(',');
...
pub struct File {
    ...
    pub records: ::std::vec::Vec<ResistInfoRecord>,
    pub footer: [u8; 64],
}
...
impl File {
    ...
    pub fn parse(buf: &[u8], offset: &mut usize) -> ::std::result::Result<Self, Error> {
        ...
        let records = {
            ...
            for _ in 0..count {
                values.push(ResistInfoRecord::parse(buf, offset)?);
            }
            values
        };
        let footer = {
