...
std::vector<std::byte> createMinimalSave(int32_t campaignIndex = 0, int unitCount = 1) {
    ...
    // Context block: 0x438 bytes. First u32 = 0xFFFFFFFF for heuristic detection.
...
TEST_CASE("SaveFormat handles data without size prefix", "[save]") {
    ...
    for (int i = 0; i < 20; ++i) appendI32(data, 0);
    appendI32(data, 0);
...
TEST_CASE("SaveFormat handles data without context block", "[save]") {
    // Build data with prefix but no context: prefix + magic + campaign(0) + rest.
    std::vector<std::byte> data;
...
TEST_CASE("SaveFormat roster round-trip", "[save]") {
    ...
    // Roster: 2 entries.
    appendU32(data, 2);
    // Entry 0: byte_61=10, byte_60=20, byte_62=30, byte_63=40, value_64=100.
    appendU8(data, 10);
    appendU8(data, 20);
    appendU8(data, 30);
    appendU8(data, 40);
...
TEST_CASE("SaveFormat mission completion round-trip", "[save]") {
    ...
    // Offset: prefix(4) + magic(4) + context(0x438) + campaign(4) + mainblock(0x154) + unitcount(4) + 1 unit(483) + selected(4) + roster_count(4) + second_count(4)
    // = 4 + 4 + 1080 + 4 + 340 + 4 + 483 + 4 + 4 + 4 = 1931
    // Then 20 × i32 mission values start at offset 1931.
    constexpr size_t missionOffset = 4 + 4 + kuf::kSaveContextSize + 4 + kuf::kSaveMainBlockSize + 4 + kuf::kSaveUnitSize + 4 + 4 + 4;
    int32_t val5 = 5;
    int32_t val10 = 10;
    std::memcpy(data.data() + missionOffset, &val5, 4);      // slot 0
    std::memcpy(data.data() + missionOffset + 19 * 4, &val10, 4);  // slot 19

    // Re-patch size prefix since data was modified (values unchanged in size).
    uint32_t totalSize = static_cast<uint32_t>(data.size());
    std::memcpy(data.data(), &totalSize, 4);
