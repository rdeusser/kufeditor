...
std::vector<std::byte> createMinimalTroopInfoSox() {
	...
	// First troop: set some recognizable values (file stores integers, not
	// floats).
...
TEST_CASE("SoxBinary parses troop fields", "[sox_binary]") {
	...
	REQUIRE_THAT(troop.moveSpeed,
		     Catch::Matchers::WithinAbs(130.0f, 0.001f));
	REQUIRE_THAT(troop.resistMelee,
		     Catch::Matchers::WithinAbs(100.0f, 0.001f));
	REQUIRE_THAT(troop.defaultUnitHp,
...
TEST_CASE("SoxBinary validates resistance ranges", "[sox_binary]") {
	...
	// Set invalid resistance (outside typical 0-500 range, not in
	// instant-death range).
