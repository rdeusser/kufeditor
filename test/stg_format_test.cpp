...
void appendFixedString(std::vector<std::byte> &v, const char *str,
		       size_t fixedLen) {
	size_t pos = v.size();
...
std::vector<std::byte> createMinimalStg() {
	std::vector<std::byte> data(kuf::kStgHeaderSize + kuf::kStgUnitSize,
				    std::byte{0});
...
void appendAreaEntry(std::vector<std::byte> &v, const char *desc,
		     uint32_t areaId, float x1, float y1, float x2, float y2) {
	size_t start = v.size();
...
// Build a valid variable-length tail with: 0 area IDs, 0 variables, N events in
// 1 block, 0 footer entries.
std::vector<std::byte>
createTail(const std::vector<std::vector<std::byte>> &eventBlobs) {
...
std::vector<std::byte>
buildEventBlob(const char *desc, uint32_t eventId,
...
TEST_CASE("StgFormat parses unit core data", "[stg]") {
	...
	REQUIRE_THAT(unit.leaderHpOverride,
		     Catch::Matchers::WithinAbs(-1.0f, 0.001f));
	REQUIRE_THAT(unit.unitHpOverride,
		     Catch::Matchers::WithinAbs(-1.0f, 0.001f));
	REQUIRE_THAT(unit.positionX,
		     Catch::Matchers::WithinAbs(5000.0f, 0.001f));
	REQUIRE_THAT(unit.positionY,
		     Catch::Matchers::WithinAbs(3000.0f, 0.001f));
...
TEST_CASE("StgFormat parses unit configuration", "[stg]") {
	...
	for (int i = 0; i < 22; ++i) {
		REQUIRE_THAT(unit.statOverrides[i],
			     Catch::Matchers::WithinAbs(-1.0f, 0.001f));
	}
}
...
TEST_CASE("StgFormat validates duplicate unique IDs", "[stg]") {
	...
	std::byte *unit1 =
	    data.data() + kuf::kStgHeaderSize + kuf::kStgUnitSize;
	...
	for (const auto &issue : issues) {
		if (issue.field == "uniqueId" &&
		    issue.severity == kuf::Severity::Error) {
			foundDuplicate = true;
			break;
		}
	}
...
TEST_CASE("StgFormat parses events from tail", "[stg][events]") {
	...
	auto blob =
	    buildEventBlob("Make Regnier Immortal", 1, {cond}, {act1, act2});
...
TEST_CASE("StgFormat event round-trip with modifications", "[stg][events]") {
	...
	REQUIRE(stg2.eventBlocks()[0].events[0].actions[0].params[0].intValue ==
		42);
...
TEST_CASE("StgFormat parses area entries", "[stg][areas]") {
	...
	appendU32(tail, 2);
	appendAreaEntry(tail, "spawn_zone", 0, 1000.0f, 2000.0f, 3000.0f,
			4000.0f);
	...
	REQUIRE_THAT(stg.areas()[0].boundX1,
		     Catch::Matchers::WithinAbs(1000.0f, 0.001f));
	REQUIRE_THAT(stg.areas()[0].boundY1,
		     Catch::Matchers::WithinAbs(2000.0f, 0.001f));
	REQUIRE_THAT(stg.areas()[0].boundX2,
		     Catch::Matchers::WithinAbs(3000.0f, 0.001f));
	REQUIRE_THAT(stg.areas()[0].boundY2,
		     Catch::Matchers::WithinAbs(4000.0f, 0.001f));

	REQUIRE(stg.areas()[1].description == "objective");
	REQUIRE(stg.areas()[1].areaId == 1);
	REQUIRE_THAT(stg.areas()[1].boundX1,
		     Catch::Matchers::WithinAbs(500.0f, 0.001f));
	REQUIRE_THAT(stg.areas()[1].boundY2,
		     Catch::Matchers::WithinAbs(800.0f, 0.001f));
}
...
TEST_CASE("StgFormat area modification round-trip", "[stg][areas]") {
	...
	REQUIRE_THAT(stg2.areas()[0].boundX1,
		     Catch::Matchers::WithinAbs(1111.0f, 0.001f));
}
