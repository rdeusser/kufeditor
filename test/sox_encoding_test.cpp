...
TEST_CASE("soxDecode converts ASCII hex to bytes", "[sox_encoding]") {
	// ASCII "64000000" should decode to bytes 0x64 0x00 0x00 0x00
	// (little-endian 100).
...
TEST_CASE("soxDecode handles mixed case", "[sox_encoding]") {
	...
	std::vector<std::byte> encoded = {std::byte{'A'}, std::byte{'b'},
...
TEST_CASE("soxEncode converts bytes to ASCII hex", "[sox_encoding]") {
	std::vector<std::byte> binary = {std::byte{0x64}, std::byte{0x00},
...
TEST_CASE("soxEncode/soxDecode round-trip", "[sox_encoding]") {
	...
	REQUIRE(std::memcmp(decoded->data(), original.data(),
			    original.size()) == 0);
}
