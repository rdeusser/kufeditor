...
std::vector<std::byte> createSkillInfoSox(int32_t id, const std::string &locKey,
...
TEST_CASE("SoxSkillInfo parses header correctly", "[sox_skill_info]") {
	...
	auto data =
	    createSkillInfoSox(0, "@(S_Melee)", "IL_SKL_Melee.tga", 1, 50);
...
TEST_CASE("SoxSkillInfo handles negative skill ID", "[sox_skill_info]") {
	kuf::SoxSkillInfo sox;
	auto data =
	    createSkillInfoSox(-2, "@(S_Elemental)", "IL_SKL_Elem.tga", 2, 25);
...
TEST_CASE("SoxSkillInfo validates invalid skill type", "[sox_skill_info]") {
	kuf::SoxSkillInfo sox;
	auto data =
	    createSkillInfoSox(0, "@(S_Melee)", "IL_SKL_Melee.tga", 5, 50);
	...
	for (const auto &issue : issues) {
		if (issue.field == "skillType" &&
		    issue.severity == kuf::Severity::Warning) {
...
TEST_CASE("SoxSkillInfo validates zero max level", "[sox_skill_info]") {
	kuf::SoxSkillInfo sox;
	auto data =
	    createSkillInfoSox(0, "@(S_Melee)", "IL_SKL_Melee.tga", 1, 0);
